	...
	, RestartTime(10000)
{
	...
	//-----------アニメーションを読み込み-----------------//
	// アイドリングアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_IDLE)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Happy_Idle_Anim.gpanim", true);
	// 一定以上入力がなかった際のアイドリングアニメーション（ダンス）
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_IDLE_DANCE)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Tut_Hip_Hop_Dance.gpanim", true);
	// 走りアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_RUN)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Running.gpanim", true);
	// 走りだしアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_RUN_START)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Idle_To_Sprint_2.gpanim", false);
	// 走り終わりアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_RUN_STOP)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Run_To_Stop.gpanim", false);
	// 走り中の切り替えしアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_RUN_TURN)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Change_Direction.gpanim", false);
	// 敵に当たった際のノックバックアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_KNOCKBACK)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Receiving_An_Uppercut.gpanim", false);
	// ジャンプループアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_JUMPLOOP)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Floating.gpanim", true);
	// ジャンプ開始アニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_JUMPSTART)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Jump_up.gpanim", false);
	// 次の状態が待機の時の着地アニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_JUMPEND_TO_IDLE)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Landing.gpanim", false);
	// 次の状態が走りの時の着地アニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_JUMPEND_TO_RUN)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Falling_To_Roll.gpanim", false);
	// タイムオーバー時のアニメーション（start）
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_DOWNSTART)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Praying_down.gpanim", false);
	// タイムオーバー時のアニメーション（loop）
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_DOWN_LOOP)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Praying_Idle.gpanim", false);
	// タイムオーバー時のアニメーション（コンティニュー）
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_DOWN_UP)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Praying_up.gpanim", false);
	// タイムオーバー時のアニメーション（ゲームオーバー）
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_DOWN_OVER)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/over_down.gpanim", false);
	// 死亡時のアニメーション
	animTypes[static_cast<unsigned int>(PlayerState::PLAYER_STATE_DEAD)] = GRAPHICS_RESOURCE->CreateAnimation("Assets/Model/Player/animation/Stunned.gpanim", false);
